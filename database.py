...
class Database:
    ...
    def get_queued_jobs_count(self):
        """Get count of queued + processing jobs"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_banned_users(self):
        """Get all banned users with full info"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def ban_user(self, user_id) -> bool:
        """Ban a user - returns True if updated"""
        conn = self.get_connection()
        cursor = conn.cursor()

        cursor.execute('UPDATE users SET is_banned = 1 WHERE user_id = ? AND is_banned = 0', (user_id,))
    ...
    def unban_user(self, user_id) -> bool:
        """Unban a user - returns True if updated"""
        conn = self.get_connection()
        cursor = conn.cursor()

        cursor.execute('UPDATE users SET is_banned = 0 WHERE user_id = ? AND is_banned = 1', (user_id,))
        updated = cursor.rowcount > 0
    ...
    def get_conversion_stats(self):
        ...
        cursor.execute("SELECT COUNT(DISTINCT user_id) FROM conversion_jobs WHERE created_at >= datetime('now', '-1 day')")
        daily_active_users = cursor.fetchone()[0]

        cursor.execute("SELECT COUNT(DISTINCT user_id) FROM conversion_jobs WHERE created_at >= datetime('now', '-7 days')")
